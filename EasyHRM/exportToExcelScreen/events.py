...
def create_event_interface(settings_frame):
    ...
    # Reset Events Button
    reset_button = ctk.CTkButton(settings_frame, text="Reset Events", command=reset_events)
...
def show_comments(settings_frame):
    ...
    for key, value in sorted(commentsDict.items()):
        ...
        text_label = ctk.CTkLabel(comment_frame, text=f"Event: {value} - Time: {convertTimeToText(key)}", font=("Arial", 12, "bold"))
